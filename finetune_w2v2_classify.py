...
os.environ["HF_METRICS_CACHE"] = "/datahuggingface/metrics"
...
class DataTrainingArguments:
    ...
    dataset_path: str = field(
        default=None, metadata={"help": "The path of the dataset, including train and dev)."}
    )
    wav_root_dir: str = field(
        default=None, metadata={"help": "The dir of the wav files,e.g. /data/MGTV/train"}
    )
...
class DataCollatorCTCWithPadding:
    ...
    # denoiser: demucs.apply.BagOfModels
    padding: Union[bool, str] = True
    max_length: Optional[int] = None
    max_length_labels: Optional[int] = None
    pad_to_multiple_of: Optional[int] = None
    pad_to_multiple_of_labels: Optional[int] = None
    wav_root_dir: Optional[str] = "/data/MGTV/train"
    ...
    def __call__(self, features: List[Dict[str, Union[List[int], torch.Tensor]]]) -> Dict[str, torch.Tensor]:
        ...
        if "speech" in features[0].keys():
            ...
            for feature in features:
                waveform = sox_effects.apply_effects_file(path=feature["file"].replace("/data/MGTV/train", self.wav_root_dir), effects=[['rate', str(16000)]])[0]
                waveform = waveform[0]
        ...
        if "labels" in features[0].keys():
            ...
            assert ("label" in features[0].keys()),f" ['labels'] or ['text'] is required for training"
            class_list = ['angry', 'fear', 'happy', 'neutral', 'sad', 'surprise']
            batch["labels"] = torch.tensor([class_list.index(feature["label"]) for feature in features], dtype=torch.long)
...
class DataCollatorCTCWithPadding_Speed_Perturb:
    ...
    add_noise_reverb = Add_Noise_Reverb(musan_path="/data/MGTV/music_noise/",
                                        rir_path="/data/RIRS_NOISES/") if add_no_re else None
        ...
        if "speech" in features[0].keys():
            for feature in features:
                ...
                if self.add_no_re:# and speed == 1.0:
                    # 1-4则加混响或者加噪
                    augtype = random.choices([0, 4], [1, 1], k=1)[0]
            ...
            for feature in features:
                speed = random.choices(self.speeds, self.sp_weights, k=1)[0]
                if speed != 1.0:
                    feature_speed_perturbed = sox_effects.apply_effects_file(path=feature["file"].replace("/data/MGTV/train", self.wav_root_dir),
                                                                             effects=[['speed', str(speed)],
                                                                                      ['rate', str(16000)]])[0]
                else:
                    feature_speed_perturbed = sox_effects.apply_effects_file(path=feature["file"].replace("/data/MGTV/train", self.wav_root_dir), effects=[['rate', str(16000)]])[0]
                if self.add_no_re and speed == 1.0:
                    # 1-4则加混响或者加噪
                    augtype = random.choices([0, 4], [1, 1], k=1)[0]
                    feature_speed_perturbed_np = feature_speed_perturbed.numpy()
...
class CTCTrainer(Trainer):
        ...
        for step, inputs in enumerate(dataloader):
            ...
            if self.eval_only_encoder:
                ...
                loss, logits, labels = self.prediction_step(model, inputs, prediction_loss_only,
                                                            ignore_keys=ignore_keys)
            # logger.info(loss, logits, labels)
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (
                    ...
                    #########################################################
                    with model.no_sync():
                        if self.eval_only_encoder or self.mtl_loss:
    ...
    def compute_mtl_loss(self, model, inputs, return_outputs=False):
        """
        ...
        # print(loss)
        return (loss, outputs) if return_outputs else (loss,ctc_loss,additional_loss)
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]],
            ...
            with autocast():
                ...
                if not teacher_model:
                    ...
                    #                         print(f"att_loss={att_loss.item()}")
                        ...
                        loss, ctc_loss, additional_loss = self.compute_mtl_loss(model, inputs)
                    else:
        ...
        if model is not self.model:
            ...
            if self.eval_only_encoder:
                ...
                if model.module.config.ctc_loss_reduction == "mean":
                    ...
                    ctc_loss = ctc_loss.sum() / inputs["labels"].shape[0]
                    additional_loss = additional_loss.sum() / inputs["labels"].shape[0]
        ...
        if self.args.gradient_accumulation_steps > 1:
            ...
            if self.eval_only_encoder:
                ...
                loss_tuple = (ctc_loss, att_loss)
            if self.mtl_loss:
                ctc_loss = ctc_loss / self.args.gradient_accumulation_steps
                additional_loss = additional_loss / self.args.gradient_accumulation_steps
                loss_tuple = (ctc_loss, additional_loss)
        if self.use_amp:
...
def main():
    ...
    # logger.info(model)
    model = Wav2Vec2ForSequenceClassification_czc.from_pretrained(model_args.model_name_or_path)
    model.config.mask_time_prob = 0.50
    model.config.mask_time_length = 10
    model.config.mask_feature_prob = 0.50
    model.config.mask_feature_length = 64
    model.config.ctc_zero_infinity = True

    # print(model)
    # print(vars(model.config))
    # state_dict = torch.load("/home/work/training_output/olr2021/62.0-28.2-checkpoint-51000/pytorch_model.bin",map_location="cpu")
    # print("reload")
    # model.load_state_dict(state_dict,strict=False)

    logger.info(f"vocab_size = {processor.tokenizer.vocab_size}")
    ...
    # 1000、100、10小时
    # train_dataset = load_from_disk(prepare_train_path).select(range(86666))
    train_dataset = load_from_disk(prepare_train_path)#.select(range(8333))

    train_dataset_vocal = load_from_disk("/work/MGTV/hf_datasets/train_vocal")

    val_dataset = load_from_disk(prepare_dev_path)

    train_dataset = datasets.concatenate_datasets([train_dataset, train_dataset_vocal])

    ignored_columns = list(set(train_dataset.column_names) - set(["file", "text", "length", "label", "speed", "volumn"]))
    train_dataset = train_dataset.remove_columns(ignored_columns)
    val_dataset = val_dataset.remove_columns(ignored_columns)
    ...
    if data_args.speed_perturb:
        data_collator = DataCollatorCTCWithPadding_Speed_Perturb(processor=processor,padding=True, wav_root_dir=data_args.wav_root_dir)
    else:
        data_collator = DataCollatorCTCWithPadding(processor=processor, padding=True, wav_root_dir=data_args.wav_root_dir)

    # 验证集不进行在线增强
    data_collator_eval = DataCollatorCTCWithPadding(processor=processor, padding=True, wav_root_dir=data_args.wav_root_dir)
    ...
    def compute_metrics_f1(pred):
        ...
        result_mat = torch.zeros((6,6), dtype=torch.long)
        label_ids = pred.label_ids
        # print(label_ids)
        for label,pred in zip(label_ids,pred_ids):
            result_mat[label,pred] += 1 
        logger.info(f'result_mat: \n {result_mat}')
        # acc = result_mat.diag().sum()/result_mat.sum()

        acc = f1_score(label_ids, pred_ids, average='micro') 
        logger.info(f'acc: {acc}')
        # err = 1 - acc
        return {"eval_f1score": acc}
    ...
    trainer = CTCTrainer(
        ...
        teacher_model=None,
        eval_only_encoder=False,
        use_w2v2_lr_scheduler=model_args.use_w2v2_lr_scheduler,
        use_czc_lr_scheduler=model_args.use_czc_lr_scheduler,
        mtl_loss=False,
    )
